    ...
    <div className="flex flex-col w-full h-full bg-[#fafafa] text-slate-800 overflow-hidden font-sans select-none">
      ...
      <div className="flex items-center justify-between px-3 py-2 bg-white border-b border-slate-200/90 text-xs shrink-0">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            onClick={() => setZoom(1)}
            className="px-1.5 py-1 text-[10px] font-mono rounded hover:bg-slate-100 text-slate-600 transition"
            title="Reset Zoom"
          >
            {Math.round(zoom * 100)}%
          </button>
      ...
      <div ref={containerRef} className="flex-1 w-full h-full relative overflow-auto p-6 flex items-center justify-center bg-white/70">
        {error ? (
          <div className="max-w-md p-4 bg-rose-50 border border-rose-200 rounded-xl text-rose-800 text-xs flex items-start gap-2.5">
            <AlertCircle size={16} className="text-rose-600 shrink-0 mt-0.5" />
            <div>
              <div className="font-bold mb-1">Gagal merender diagram Mermaid:</div>
