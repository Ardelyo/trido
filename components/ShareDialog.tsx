          ...
          >
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50/50">
              <h3 className="text-[16px] font-extrabold text-slate-800">
                {t('share', 'Bagikan')} {t('whiteboard', 'Papan Tulis')}
              </h3>
            ...
            <div className="p-6 flex flex-col gap-4">
              ...
              <div className="text-center">
                <div className="text-[17px] font-extrabold text-slate-900 mb-1">{t('activeSyncSession', 'Sesi Sinkronisasi Aktif')}</div>
                <div className="text-[13px] text-slate-500 font-medium px-2 leading-relaxed">
                  {t('shareSessionDesc', 'Bagikan tautan unik ini. Audiens dapat melihat pergerakan layar dan papan tulis Anda secara langsung.')}
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div className="text-[11px] uppercase tracking-wider font-bold text-slate-400 px-1">{t('shareLink', 'Tautan Berbagi')}</div>
                <div className="flex items-center gap-2 p-1.5 bg-white rounded-[1.25rem] border border-slate-200">
                   ...
                   >
                     {copied ? (
                       <><CheckCircle2 size={16} /> <span>{t('copied', 'Tersalin')}</span></>
                     ) : (
                       <><Copy size={16} /> <span>{t('copy', 'Salin')}</span></>
                     )}
                   </button>
                </div>
              </div>
