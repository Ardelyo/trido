...
import { X, Search, Clock, Calendar, MoreVertical, Trash2, Edit2, Share2, FileText, Download } from 'lucide-react';
      ...
      <div className="flex-1 overflow-y-auto p-8 lg:p-12 w-full max-w-7xl mx-auto custom-scrollbar">
         ...
         {filtered.length > 0 ? (
                ...
                {filtered.map((item, idx) => (
                   ...
                   >
                      <div className="h-40 bg-zinc-100 relative overflow-hidden">
                         <img src={item.thumbnail} alt={item.title} className="w-full h-full object-cover opacity-90 group-hover:opacity-100 transition-opacity group-hover:scale-105 duration-500" />
                         ...
                         <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1">
                           <button className="w-8 h-8 rounded-lg bg-white/90 backdrop-blur-sm text-zinc-600 flex items-center justify-center hover:bg-blue-50 hover:text-blue-600 transition-colors shadow-sm">
                             <Share2 size={14} />
                           </button>
                           <button 
                             onClick={(e) => { e.stopPropagation(); deleteSession(item.id); }}
                             className="w-8 h-8 rounded-lg bg-white/90 backdrop-blur-sm text-zinc-600 flex items-center justify-center hover:bg-rose-50 hover:text-rose-600 transition-colors shadow-sm"
                           >
                             <Trash2 size={14} />
                           </button>
                         </div>
                      ...
                      <div className="p-5 flex flex-col flex-1">
                         ...
                         <div className="mt-auto grid grid-cols-2 gap-2">
                           <button className="py-2.5 px-3 rounded-xl bg-zinc-50 hover:bg-zinc-100 text-zinc-700 font-bold text-sm flex items-center justify-center gap-2 transition-colors border border-zinc-200">
             ...
             <div className="flex flex-col items-center justify-center h-64 text-center">
                ...
                <h3 className="text-xl font-black text-zinc-700 mb-2">Papan Tulis tidak ditemukan</h3>
                <p className="text-zinc-500 font-medium">Coba gunakan nama file yang berbeda.</p>
             </div>
         )}
      </div>
    </motion.div>
